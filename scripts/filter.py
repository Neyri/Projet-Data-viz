...
all_activities = ['IN_ROAD_VEHICLE', 'UNKNOWN',
                  'IN_RAIL_VEHICLE', 'WALKING', 'STILL', 'ON_BICYCLE', 'RUNNING']
...
for d in data:
    if 'activity' in d.keys():
        ...
        missing_activities = list(
            set(all_activities) - set(existing_activities))
        for missing_activity in missing_activities:
            activity_final.append(
                {'type': missing_activity, 'confidence': 0})
